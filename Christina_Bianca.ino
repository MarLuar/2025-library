...
BLYNK_WRITE(V0) {
  ...
  if (currentTime - lastButtonPressTime > debounceDelay) {
    ...
    if (param.asInt() == 1) {  // If V0 is HIGH
      scores[ZONE_1][0]++;     // Increment zone1 score1
...
// Blynk Virtual Pin V9 handler (toggle timeout)
BLYNK_WRITE(V9) {
...
void setup() {
  ...
  setupZones();
  delay(500);
  P.displayAnimate();  // Start animation
...
void loop() {
  ...
  // Timeout LED control (turn off after 3 seconds)
...
// Function to update Zone 4 with timer values
